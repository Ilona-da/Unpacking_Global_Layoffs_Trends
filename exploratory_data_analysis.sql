...
ADD year INT, month INT, quarter INT
...
WITH running_total_cte AS (
	SELECT year, month, SUM(total_laid_off) AS sum_laid_off
	FROM layoffs_staging
	WHERE DATE IS NOT NULL
	GROUP BY year, month
)

SELECT year, month, sum_laid_off, SUM(sum_laid_off) OVER(ORDER BY year, month) AS running_total
...
WITH company_year AS (
	SELECT company, industry, year, SUM(total_laid_off) AS sum_laid_off
	FROM layoffs_staging
...
SELECT company, industry, year, sum_laid_off, ranking
FROM company_year_ranking_cte
WHERE ranking <= 5
ORDER BY year, ranking

-- total layoffs by year and quarters
SELECT year, quarter, SUM(total_laid_off) AS sum_laid_off
FROM layoffs_staging
...
SELECT year, month, SUM(total_laid_off) AS total_laid_off,
    LAG(SUM(total_laid_off)) OVER(ORDER BY year, month) AS prev_month_laid_off,
    SUM(total_laid_off) - LAG(SUM(total_laid_off)) OVER(ORDER BY year, month) AS change_from_prev_month
FROM layoffs_staging
GROUP BY year, month
ORDER BY year, month
